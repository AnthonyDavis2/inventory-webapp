<template>
  <div>
    ...
    <div class="flex gap-3 mb-4">
      <USelect v-model="statusFilter" :options="statusOptions" option-attribute="label" value-attribute="value" placeholder="All statuses" class="w-48" @change="fetchOrders" />
    </div>
...
async function fetchOrders() {
  loading.value = true
  try {
    const data = await request<{ data: any[]; total: number }>('/purchasing/orders', { query: { status: statusFilter.value, page: page.value, limit: perPage } })
